...
def sum_light(els: List[datetime], start_watching: Optional[datetime] = None,
    ...
    for i in range(0, len(els), 2):
        if start_watching == None:
            ...
            b += a.total_seconds()
            continue
        if (els[i] - start_watching).total_seconds() <= 0 and (els[i + 1] - start_watching).total_seconds() > 0 and (end_watching - els[i+1]).total_seconds() > 0:
            a = els[i + 1] - start_watching
            b += a.total_seconds()
            c = 1
            continue
        if (end_watching - els[i+1]).total_seconds() <= 0:
            a = end_watching - els[i]
            continue
        if c == 1:
            a = els[i + 1] - els[i]
            b += a.total_seconds()
